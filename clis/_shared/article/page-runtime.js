...
var PP = (function () {
  ...
  // 知乎 zhuanlan 实测如此），所以 data: URI 必须用 atob 手动解码成字节。
  ...
  //（Joseph Myers md5.js），改为吃 ArrayBuffer/Uint8Array/string，输出 hex。
  function md5(input) {
    var bytes;
    ...
    function blockAt(offset) {
      var blks = new Array(16);
      for (var i = 0; i < 16; i++) {
        blks[i] = bytes[offset + i * 4]
    ...
    var i;
    for (i = 64; i <= n; i += 64) md5cycle(state, blockAt(i - 64));
    var base = i - 64;           // 尾块起点（必须在 i 被复用前取）
    var rem = n - base;          // 尾块剩余字节数（0..63）
    // 尾块：剩余字节 + 0x80 填充 + 长度（bit，little-endian）
    var tail = new Array(16); for (i = 0; i < 16; i++) tail[i] = 0;
    for (i = 0; i < rem; i++) tail[i >> 2] |= bytes[base + i] << ((i % 4) << 3);
    tail[i >> 2] |= 0x80 << ((i % 4) << 3);
    if (i > 55) {
      md5cycle(state, tail);
      for (i = 0; i < 16; i++) tail[i] = 0;
    }
    // 长度以 bit 计，最大安全整数内拆高低位
    tail[14] = (n * 8) & 0xFFFFFFFF;
    tail[15] = Math.floor((n * 8) / 4294967296) & 0xFFFFFFFF;
    md5cycle(state, tail);

    var hex = '';
    for (i = 0; i < 4; i++) {
      for (var j = 0; j < 4; j++) {
        hex += ((state[i] >> (j * 8 + 4)) & 0x0F).toString(16) + ((state[i] >> (j * 8)) & 0x0F).toString(16);
      }
    }
    return hex;
  }
